...
def trade(context, data):
    for stock in context.stocks1:
        if data.can_trade(stock) and not data.is_stale(stock):
            ...
            try:
                macd, signal, hist = talib.MACDEXT(prices, fastperiod=12, fastmatype=context.fastma, slowperiod=26, slowmatype=context.slowma, signalperiod=9, signalmatype=context.signalma)
            except:
                context.stocks1.remove(stock)
                log.info(stock)
                continue

            macdval= macd[-1] - signal[-1]
            position = context.portfolio.positions[stock].amount
            
            if macdval > 0 and position == 0:
                order_target_percent(stock, percent)

            elif macdval < 0 and position > 0:
