...
def trade(context, data):
    for stock in context.stocks1:        
        if data.can_trade(stock) and not data.is_stale(stock):
            ...
            try:
                upper, middle, lower = talib.BBANDS(prices, timeperiod=10, nbdevup=2, nbdevdn=2, matype=context.matype)    
            except:
                context.stocks1.remove(stock)
                log.info(stock)
                continue

            if price <= lower[-1] and position <= 0:
                order_target_percent(stock, percent)        
            
            elif price >= upper[-1] and position >= 0:
