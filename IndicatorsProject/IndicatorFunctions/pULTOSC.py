...
def trade(context, data):
    for stock in context.stocks1:
        if data.can_trade(stock) and not data.is_stale(stock):
            ...
            closes= data.history(stock, 'price', 30, '1d')
            lows= data.history(stock, 'low', 30, '1d')
            try:
                ultosc= talib.ULTOSC(highs, lows, closes, timeperiod1=7, timeperiod2=14, timeperiod3=28)[-1]
            except:
            ...
            if ultosc > context.highosc and position > 0:
                order_target_percent(stock, 0)

            elif ultosc < context.lowosc and position == 0:
